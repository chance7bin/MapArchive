...
public class MetadataServiceImpl implements IMetadataService {
    ...
    public Map<String, Object> getMetadataByFilenameByType(String filename, String mapCLSId, String mapType, String excelPath) {
        ...
        try {
            if (collection.contains("BASIC_SCALE_MAP")){        // 符合基本比例尺命名作为一个大类匹配
                int index = collection.lastIndexOf("MAP") + 4;
                String type = collection.substring(index);      // 获取类型
                Map<String,String> mapInfo = new HashMap<>();
                mapInfo.put("type",type);
                mapInfo.put("matchField","原图幅编号");
                mapInfo.put("matchFieldAndYear","numAndYear");
                return getBSMMetadata(filename,mapInfo,collection,mapType,excelPath);
            }else {     // 其余地图按新添的规则进行分类
    ...
    //得到文件名
    @Override
    public String get_english_name(String name, Boolean delBlock){
        name = name.replace('（', '(');
        name = name.replace('）', ')');
        name = name.replace('，', ',');
        name = name.replace('、', ',');
    ...
    @Override
    public String leftPad(int length, String number) {
...
//    }
    ...
    // 不只是基本比例尺，适用于按标准图幅规范的图名
    public Map<String, Object> getBSMMetadata(String filename, Map<String,String> mapInfo, String collection, String mapType, String excelPath) throws Exception{
        // 获取类型对应需要的匹配参数
        String type = mapInfo.get("type");
        String matchField = mapInfo.get("matchField");
        String matchFieldAndYear = mapInfo.get("matchFieldAndYear");
        ...
        // 得到文件名
        filename = filename.substring(0, filename.lastIndexOf("."));//出去文件名后缀
        filename = get_english_name(filename,true);
...
//            }
            Pattern p = Pattern.compile("[\u4e00-\u9fa5]");             // 判断是否包含中文
            Matcher m = p.matcher(filename);
            if(m.find()){       // 南通市(5).tif\07-50-宜-1  正常：05-49-027-A-1-(1).tif
                return matchData("原图幅编号", "numAndYear", mapType,excelPath,filename,collection);
            }
        }

        //将(1)(2)(3)(4)转换为<1><2><3><4>,以免和年份混淆
        if(replaceIndex(type, filename) == null){
            return null;
        }
        filename = replaceIndex(type, filename);
        ...
        // 进行图名规范化
        if(basicMapFormat(type, name_without_year, name_after_year, collection) == null){
            return null;
        }
        String formatFilename = basicMapFormat(type, name_without_year, name_after_year, collection);

        // 根据文件名匹配元数据
        return matchData(matchField,matchFieldAndYear,mapType,excelPath,formatFilename,collection);
    }
...
//    }
    ...
    //将(1)(2)(3)(4)转换为<1><2><3><4>,以免和年份混淆
    public String replaceIndex(String type, String filename){
        if(type.equals("TWENTY") || type.equals("ONE")){        // 20w: 02-47-(4).tif   1w: 05-49-027-A-1-(1).tif
            int oldCount = filename.length();
            int newCount = filename.replace("(","").length();
...
//            name_after_year = name_without_year.substring(i) + name_after_year;     //按道理name_after_year一开始应该是空的
    ...
    // 基本比例尺文件名规范化
    public String basicMapFormat(String type, String name_without_year, String name_after_year, String collection){
        String formatFilename = null;
        String new_name_str = "";   //为之后拼幅做迭代
        if (name_without_year.contains(".") || name_without_year.contains(",")){  //说明是拼幅
            String[] blocks ;
            ...
            for (String b : blocks) {
                ...
                //说明没有完整的，只是最后一个数字
                    ...
                    int first_square_bracket;
                    int second_square_bracket;
                    if (type.equals("TEN")) {          // 06-51-064.063
                        String block_name = leftPad(3, b);
                        new_name_str = new_name_str + '.' + block_name;
                    } else if (type.equals("HUNDRED")) {       // 21-33.34
                        String block_name = leftPad(2, b);
                        new_name_str = new_name_str + '.' + block_name;
                    } else if (type.equals("TWENTY_FIVE")) {          // 12-46-[12].[13]
                        first_square_bracket = b.indexOf('[');
                        second_square_bracket = b.indexOf(']');
                        if (second_square_bracket < 0) {
                            second_square_bracket = b.length();
                        ...
                        String block_name = leftPad(2, b);
                        new_name_str = new_name_str + ".[" + block_name + ']';
                    } else if (type.equals("TWENTY")) {               // 11-52-(22).(23)
                        first_square_bracket = b.indexOf('(');
                        second_square_bracket = b.indexOf(')');
                        if (second_square_bracket < 0) {
                            second_square_bracket = b.length();
