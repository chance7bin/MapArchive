...
public class FileUtils {
     ...
     * @Author bin
     **/
    public static boolean uploadSingleFile(MultipartFile file, String inputDir, String md5) {
        ...
        String path = inputDir + "/" + originalFilename;
    ...
    public static List<JSONObject> zipUncompress(String inputDir, String destDir) throws Exception {
        ...
        log.info(inputDir + ", zip uncompress success");
        return fileList;
    }
    ...
    public static void toZip(List<File> srcFiles , String tempPath)throws RuntimeException {
        ...
        try {
            ...
            // zos = new ZipOutputStream(out); //这个要放到循环里，不然要是文件名一样的话处理完第一个文件zos就被关了
     ...
     * <p>
     * 用户模式下：
     * 弊端：对于少量的数据可以，但是对于大量的数据，会造成内存占据过大，有时候会造成内存溢出
     * 建议修改成事件模式
     */
    public static List<Map<String, Object>> redExcel(String filePath) throws Exception {
...
//                }
    ...
    public static List<String[]> readCSV(String csvName) throws IOException {
        ...
        CSVReader csvReader = new CSVReader(new InputStreamReader(dataInputStream,"GBK"));
     ...
     * @return
     */
    public static JSONObject csv2JSON(String[] keys,String[] values) {
     ...
     * @return
     */
    public static JSONObject[] csv2JSON(String[] keys,List<String[]> stringsList) {

        JSONObject[] jsons = new JSONObject[stringsList.size()];
        int index = 0 ;
        for (String[] strings : stringsList
        ) {
            JSONObject json = csv2JSON(keys, strings);
            jsons[index] = json;
            index ++ ;
        }
        return jsons;
     ...
     * @Author bin
     **/
    public static String getFilenameNoSuffix(File inputFile){
